...
-(void)setModelArr:(NSMutableArray *)modelArr{
    ...
    for (int i=0; i<modelArr.count; i++) {
        ...
        if (model.isVideo==YES) {
          UIImage *logoimg=[UIImage imageNamed:@"Viewer_Logo"];
           UIImage *img = [UIImage OriginImage:logoimg scaleToSize:CGSizeMake(142.0, 80.0) withLogoImage:ImgNamed(@"play") scale:4];
            
            [imgView setCurrentImage:img withURL:model.pathUrl withQueue:self.bottonSroqueue withSize:CGSizeMake(142.0, 80.0) withScale:4.0];
            
        }else{
            if ([model.pathUrl.description hasPrefix:@"http"]) {
                imgView.image=[UIImage imageNamed:@"Viewer_Logo"];
