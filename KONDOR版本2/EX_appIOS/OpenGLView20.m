...
#pragma mark - 接口
- (void)displayYUV420pData:(void *)WTFrame width:(NSInteger)w height:(NSInteger)h
{
    //_pYuvData = data;
    ...
    @synchronized(self)
    {
//        GLsizei newh=h; GLsizei neww=w;
          GLsizei nh=[[NSNumber numberWithInteger:h] intValue];
          GLsizei nw=[[NSNumber numberWithInteger:w] intValue];
        if (w != _videoW || h != _videoH)
        {
            [self setVideoSize:nw height:nh];
        }
        [EAGLContext setCurrentContext:_glContext];
        
        glBindTexture(GL_TEXTURE_2D, _textureYUV[TEXY]);
      
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, nw, nh, GL_RED_EXT, GL_UNSIGNED_BYTE, WTFrame);
        
        //[self debugGlError];
        
        glBindTexture(GL_TEXTURE_2D, _textureYUV[TEXU]);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, nw/2, nh/2, GL_RED_EXT, GL_UNSIGNED_BYTE, WTFrame+nw*nh);
//
//       // [self debugGlError];
//        
        glBindTexture(GL_TEXTURE_2D, _textureYUV[TEXV]);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, nw/2, nh/2, GL_RED_EXT, GL_UNSIGNED_BYTE, WTFrame+nw*nh*5/4);
