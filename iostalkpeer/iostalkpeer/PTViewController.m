...
- (void)ioFrameChannel:(PTChannel*)channel didReceiveFrameOfType:(uint32_t)type tag:(uint32_t)tag payload:(PTData*)payload {
  ...
  NSLog(@"手机端:didReceiveFrameOfType: %u, %u, %@", type, tag, payload);
    
    if (payload!=nil) {
        
        NSString *str = [[NSString alloc]initWithData:[NSData dataWithBytes:payload.data length:payload.length] encoding:NSUTF8StringEncoding];
        
        [self appendOutputMessage:str];
